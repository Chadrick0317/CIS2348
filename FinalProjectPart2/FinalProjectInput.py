...
#Creates a csv output file for the entire inventory.
    def full(self):
        with open('./files/FullInventory.csv', 'w') as file:
...
#Creating the csv file based of type (laptop, phone, tower, etc.).
    def by_type(self):
        items = self.item_list
        types = []
...
# Manufacturer output item that match the price
                    if similar_items:
                        matched_price = price
                        closest_item = None
